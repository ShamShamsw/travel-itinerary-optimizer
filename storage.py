...
"""Persistence layer for Project 46: Travel Itinerary Optimizer."""

DATA_DIR = Path(__file__).resolve().parent / 'data'
OUTPUTS_DIR = DATA_DIR / 'outputs'
DESTINATION_LIBRARY_FILE = DATA_DIR / 'itinerary_sets.json'
RUN_CATALOG_FILE = DATA_DIR / 'runs.json'
HISTORY_FILE = DATA_DIR / 'itinerary_history.json'
...
def load_destination_library() -> List[Dict[str, Any]]:
    """Load reusable destination cases from disk."""
    ensure_data_dirs()
    return _load_list_file(DESTINATION_LIBRARY_FILE)


def save_destination_library(library: List[Dict[str, Any]]) -> str:
    """Persist destination cases to disk."""
    ensure_data_dirs()
    _save_list_file(DESTINATION_LIBRARY_FILE, library)
    return str(DESTINATION_LIBRARY_FILE)
...
def load_history() -> List[Dict[str, Any]]:
    """Load itinerary history entries from disk."""
    ensure_data_dirs()
    return _load_list_file(HISTORY_FILE)


def save_history(history: List[Dict[str, Any]]) -> str:
    """Persist itinerary history entries to disk."""
    ensure_data_dirs()
...
def save_trace_file(trace_payload: Dict[str, Any], session_id: str) -> str:
    """Persist planner trace payload for a session."""
    ensure_data_dirs()
    ...
    catalog.append(
        {
            ...
            'routes_evaluated': run_record.get('routes_evaluated', 0),
            'stops_planned': run_record.get('stops_planned', 0),
